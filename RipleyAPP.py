...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QFileDialog, QProgressBar, QPushButton, QLabel,
                             ...
                             QGroupBox, QGridLayout, QTabWidget, QSizePolicy)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.start_time = None  # 记录计算开始的时间
    ...
    def initUI(self):
        ...
        # 绑定鼠标滚轮事件，实现缩放
        self.canvas_map.mpl_connect("scroll_event", self.on_scroll)  # 滚轮放大缩小
    ...
    def on_press(self, event):
        """鼠标按下事件：当右键按下时启动平移功能"""
        if event.button == 1 and event.inaxes == self.ax_map:  # 右键（button==3）用于平移
    ...
    def update_map(self):
        ...
        if self.shanghai_map is not None:
            ...
            # 获取上海市地图的边界，以便缩放显示区域
            xmin, ymin, xmax, ymax = self.shanghai_map.total_bounds
        ...
        # 区域缩放，放大为上海市区域
        if self.shanghai_map is not None:
            # 显示上海市区域（已在上面通过 `total_bounds` 设置过范围）
            xmin, ymin, xmax, ymax = self.shanghai_map.total_bounds
            self.ax_map.set_xlim(xmin, xmax)
            self.ax_map.set_ylim(ymin, ymax)
    ...
    def start_computation(self):
        ...
        """
        # 如果当前有计算任务，取消
        if self.computation_thread is not None and self.computation_thread.isRunning():
    ...
    def update_time(self):
        """实时更新时间标签，显示耗时"""
        if self.start_time:
            elapsed = self.start_time.elapsed()  # 获取已过去的毫秒数
            seconds = elapsed // 1000  # 转换为秒
            self.label_time.setText(f"计算耗时: {seconds} 秒")
    ...
    def handle_results(self, results):
        ...
        if results['analysis_type'] =='spatial':
            ...
            ax_K = self.fig_result.add_subplot(211)
            ax_K.plot(r_values, K_values, 'b-', label='K函数')
            ax_K.set_title("K 函数")
            ax_K.set_xlabel("r")
            ax_K.set_ylabel("K(r)")
            ax_K.legend()

            ax_L = self.fig_result.add_subplot(212)
            ax_L.plot(r_values, L_values, 'r-', label='L函数')
            ...
            t_values = results["t"]
            K_st = results["K"]
            L_st = np.sqrt(K_st / (np.pi * t_values[np.newaxis,:])) - r_values[:,np.newaxis]
            # R,T = np.meshgrid(r_values,t_values)
            T, R = np.meshgrid(t_values, r_values)
            ax_K = self.fig_result.add_subplot(211,projection='3d')
            ax_K.plot_surface(R,T, K_st, cmap='viridis')
            ax_K.set_title("K 函数")
            ax_K.set_xlabel("r")
            ax_K.set_ylabel("t")
            ax_K.legend()

            ax_L = self.fig_result.add_subplot(212,projection='3d')
            ax_L.plot_surface(R, T, L_st, cmap='viridis')
            ax_L.set_title("L 函数 (sqrt(K/πt)-r)")
            ax_L.set_xlabel("r")
            ax_L.set_ylabel("t")
            ax_L.legend()
