...
class RipleyComputationThread(QThread):
    ...
    def apply_boundary_correction(self, coords, times, max_dist, max_time):
        if self.correction == "Torus":
            coords = np.vstack([coords, coords + max_dist, coords - max_dist])
            times = np.concatenate([times, times + max_time, times - max_time])
        elif self.correction == "Buffer":
            buffer_size = max_dist * 0.1
            time_buffer = max_time * 0.1
            coords = np.vstack([coords, coords + buffer_size, coords - buffer_size])
            times = np.concatenate([times, times + time_buffer, times - time_buffer])
    ...
    def run(self):
        try:
            ...
            coords, times = self.apply_boundary_correction(coords, times, max_dist, max_time)
            tree = KDTree(np.column_stack((coords, times)))
    ...
    def load_csv(self):
        ...
        if file_path:
            try:
                ...
                self.data = pd.read_csv(file_path, parse_dates=['Date']).astype({'longitude': 'float32', 'latitude': 'float32'})
                if self.data.empty:
